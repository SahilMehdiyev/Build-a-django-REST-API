...
class ProduuctListCreateAPITest(TestCase):
    ...
    def test_filter_by_min_price(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            all(float(product["price"]) >= 500 for product in response.data["results"])
        )
    ...
    def test_filter_by_max_price(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            all(float(product["price"]) <= 600 for product in response.data["results"])
        )
    ...
    def test_filter_by_public(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            all(product["public"] is True for product in response.data["results"])
        )
